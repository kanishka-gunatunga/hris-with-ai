...
foreach ($tracking_details as $tracking_detail) {
    $goal_type_id = $tracking_detail->goal_type;
    $subject = $tracking_detail->subject;
    $target_achievement = $tracking_detail->target_achievement;
    $start_date = $tracking_detail->start_date;
    $end_date = $tracking_detail->end_date;
    $discription = $tracking_detail->discription;
    $progress = $tracking_detail->progress;
    $goal_type_name = PerformanceGoalType::where('id', $goal_type_id)->value('goal_type');
}
?>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box">
                        <h4 class="mb-sm-0 dashboard-title">Edit Goal Tracking</h4>

                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item"><a href="javascript: void(0);">Dashboard</a></li>
                                <li class="breadcrumb-item active">Edit Goal Tracking</li>
                            </ol>
                        </div>
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                        ...
                        <div class="card card-theme shadow-none rounded-3 border-0"
                            ...
                            <div class="card-header border-0 bg-white p-4">
                                <h4 class="card-title mb-0 dashboard-title">Goal Details</h4>
                            </div>
                                ...
                                <div class="live-preview">
                                    <div class="row">
                                        <div class="col-md-4">
                                            <div class="mb-3">
                                                <label for="goal_type"
                                                    class="form-label text-[#556476] font-medium">Goal Type</label>
                                                <div class="custom-select">
                                                    ...
                                                    <ul class="select-dropdown hidden">
                                                        @foreach($goal_types as $type)
                                                            <li data-value="{{$type->id}}">{{$type->goal_type}}</li>
                                                        @endforeach
                                                    </ul>
                                                    <input type="hidden" name="goal_type" value="{{ $goal_type_id }}">
                                                </div>
                                                @if($errors->has("goal_type"))
                                                    <div class="alert alert-danger mt-2">{{ $errors->first('goal_type') }}
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-4">
                                            <div class="mb-3">
                                                <label class="form-label text-[#556476] font-medium">Target
                                                    Achievement</label>
                                                <input type="text" class="form-control" name="target_achievement"
                                                    value="{{ $target_achievement }}">
                                                @if($errors->has("target_achievement"))
                                                    <div class="alert alert-danger mt-2">
                                                        {{ $errors->first('target_achievement') }}
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-4">
                                            <div class="mb-3">
                                                ...
                                                @if($errors->has("progress"))
                                                    <div class="alert alert-danger mt-2">{{ $errors->first('progress') }}
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
                        ...
                        <div class="fixed-bottom bg-white border-top p-3 d-flex justify-content-between align-items-center"
                            ...
                            <button type="button" class="btn btn-outline-secondary px-4 py-2 action-btn-back"
                                ...
                                onclick="window.location.href='{{ url('performance-goal-tracking') }}'">Back</button>
                            <button type="submit" class="btn btn-theme-orange px-5 py-2">Save Goal Tracking</button>
                        </div>
                    </form>
                </div> <!-- end col -->
            </div>
